 ...
 * @LastEditors: xzz
 * @LastEditTime: 2023-03-13 08:57:07
-->
<template>

  <div class="operateHistory">
    <vxe-modal v-model="Visible" width="600px" height="560px" :position="{ top: 45 }" maskClosable
      marginSize="-600" >
      <template #title>
...
<script setup>
    ...
    // }
    const getPageData = async (e) =>{      //获取指定页数数据
      await getDatabase(e)
    }
    const  getDatabase = async (e) => {
      // await updateUserinfo()
      let {list, count} = await API.operateHistory.get({user_id: userInfo.self.userid , token: userInfo.self.userToken,page: e, limit: 10})
   ...
   onMounted(async() =>{
      ...
      await updateUserinfo()  // 必须await 获取需要时间
    ...
    //---------------当未登录时拦截操作记录-----------------
    if(userInfo.self.userid == undefined ) return
    ...
    $(document).ready(() => {  //页面文档加载完再执行查询
      ...
      node.each(function(){
        ...
        $(this).on('click',async function(){
          ...
          if(platform == '') return // 说明没有拿到平台数据,不进形记录,检查各平台app mounted是否有定义
          // console.log('platform: ', platform);
      let obj = {
        user_id: userInfo.self.userid,
        token: userInfo.self.userToken,
        platform,
        type: '开发调试,可忽略此账号记录',
        desc: $(this)[0].classList[1]
      }
      await API.operateHistory.add(obj)  //   向后端添加操作记录
        })
      })
   })
//--------------全局监听点击事件---------------end--------------------
